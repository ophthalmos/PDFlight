...
- Mehrsprachigkeit: Der **deutsche Text ist der resx-Schlüssel**. Code-Strings über `Lng.T("…")`, Designer-Texte über `Lng.Apply(form)` nach `InitializeComponent`. resx-Fallen: keine Zeilenumbrüche und keine Randleerzeichen in Schlüsseln; Ressourcennamen sind case-insensitiv (Kollisionsgefahr). Wird ein deutscher Text geändert, müssen die Schlüssel in allen Sprachdateien mitgezogen werden. Das prüft `check-lng.ps1` (MSBuild-Target `CheckTranslations`) nach jedem Build: LNG001 = deutscher Text ohne Eintrag in einer Sprachdatei, LNG002 = verwaister resx-Schlüssel. Im Debug sind das Warnungen, im **Release** Fehler mit Build-Abbruch (`-Strict`) – vor einem Release-Build oder Installer-Lauf muss der Check also sauber sein. Sprachneutrale Texte gehören in die Ignorierliste des Skripts, nicht in die resx. Das Skript ist bewusst versioniert.
- Keine `MessageBox` — immer `TaskDlg` (MsgTaskDlg/ErrTaskDlg/ConfirmTaskDlg).
- **Designer-Regel:** Sämtlicher Code, der im Visual-Studio-Designer-Inspector stehen könnte, gehört in die `.Designer.cs` (`InitializeComponent`): keine Forms komplett in Code bauen, kein Control-Aufbau, keine Property-Zuweisungen oder Event-Verdrahtungen (`FormClosing +=` usw.) im Form-Code, keine Lambdas als Event-Handler für Designer-Controls — benannte Methoden verwenden. Im Form-Code bleibt nur, was der Designer nicht kann: Laufzeitdaten, dynamisch gerenderte Bilder (ToolbarIcons), Renderer, ToolStripControlHost-Konstrukte, `Lng`-Aufrufe. Wartbarkeit über den VS-Designer hat Priorität.
- Neue Tastenkürzel in `TaskDlg.ShortcutRows` eintragen und Kürzel wie Texte in allen Sprachdateien übersetzen; F1-Dialog und Kürzel-PDF (`ShortcutsPdf`) speisen sich automatisch daraus (die README enthält bewusst keine Kürzel-Tabelle mehr).
